...
export function airMove(ctx: MovementContext, dt: number): void {
  ...
  // not just what a carry restores at takeoff (Jump.ts) — otherwise
  // air-strafe gain between hops pushes the observed peak well past the
  // intended ceiling. Surfing is a different physics path (riding a ramp's
  // geometry, not free air-strafe) and is deliberately exempt — surf speed
  // is supposed to exceed this.
  if (ctx.settings.perf.enabled && !ctx.surfing) {
